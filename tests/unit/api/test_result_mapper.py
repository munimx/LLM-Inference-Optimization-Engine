...
class TestResultMapper:
    ...
    async def test_register_and_resolve(self) -> None:
        ...
        mapper = ResultMapper()
        future = await mapper.register("req-1")
        response = _make_response("req-1")
        await mapper.resolve("req-1", response)
        result = await future
    ...
    async def test_resolve_unknown_returns_false(self) -> None:
        ...
        assert await mapper.resolve("unknown", response) is False

    async def test_reject_sets_exception(self) -> None:
        """reject() should set an exception on the future."""
        mapper = ResultMapper()
        future = await mapper.register("req-1")
        await mapper.reject("req-1", ValueError("Inference error"))
        with pytest.raises(ValueError, match="Inference error"):
            await future

    async def test_reject_unknown_returns_false(self) -> None:
        """Rejecting an unregistered request should return False."""
        mapper = ResultMapper()
        assert await mapper.reject("ghost", RuntimeError("oops")) is False

    async def test_cancel_all(self) -> None:
        """cancel_all() should cancel all pending futures."""
        mapper = ResultMapper()
        futures = [await mapper.register(f"r{i}") for i in range(3)]
        count = await mapper.cancel_all()
        assert count == 3
    ...
    async def test_pending_count(self) -> None:
        ...
        mapper = ResultMapper()
        assert mapper.pending_count == 0
        await mapper.register("r1")
        await mapper.register("r2")
    ...
    async def test_resolve_removes_from_pending(self) -> None:
        ...
        await mapper.resolve("r1", _make_response("r1"))
        assert mapper.pending_count == 0
    ...
    async def test_reject_removes_from_pending(self) -> None:
        ...
        mapper = ResultMapper()
        future = await mapper.register("r1")
        await mapper.reject("r1", RuntimeError("boom"))
        assert mapper.pending_count == 0
    ...
    async def test_cancel_all_returns_zero_when_empty(self) -> None:
        ...
        mapper = ResultMapper()
        assert await mapper.cancel_all() == 0

    async def test_multiple_resolutions_independent(self) -> None:
        """Each future resolves independently."""
        mapper = ResultMapper()
        f1 = await mapper.register("r1")
        f2 = await mapper.register("r2")
        await mapper.resolve("r1", _make_response("r1"))
        await mapper.resolve("r2", _make_response("r2"))
        resp1 = await f1
        resp2 = await f2
    ...
    async def test_is_pending_after_cancel_all(self) -> None:
        """After cancel_all(), no IDs should remain pending."""
        mapper = ResultMapper()
        await mapper.register("r1")
        await mapper.register("r2")
        await mapper.cancel_all()
    ...
    async def test_resolve_already_done_future_returns_false(self) -> None:
        ...
        result = await mapper.resolve("r1", _make_response("r1"))
        assert result is False

    async def test_pending_count_decrements_on_each_resolve(self) -> None:
        """pending_count should decrease as futures are resolved."""
        mapper = ResultMapper()
        await mapper.register("r1")
        await mapper.register("r2")
        await mapper.register("r3")
    ...
    async def test_reject_unknown_id_returns_false(self) -> None:
        """Rejecting a completely unknown ID should return False."""
        mapper = ResultMapper()
        assert await mapper.reject("nonexistent-id", Exception("oops")) is False
